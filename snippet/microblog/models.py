...
class Blog(models.Model):
    ...
    title = models.CharField(
        null=False, max_length=200, blank=False, help_text="Enter title of blog."
    )
    author = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name="blogs")
    content = models.TextField(
        max_length=1000, null=False, blank=False, help_text="Enter content of blog."
    )
...
class Comment(models.Model):
    ...
    content = models.TextField(
        null=False, max_length=1000, help_text="Enter comment about blog here"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    is_archived = models.BooleanField(default=False)
